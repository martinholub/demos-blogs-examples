...
class TrainEpisodeLogger(Callback):
    ...
    def on_episode_end(self, episode, logs):
        ...
        template += 'episode reward: {episode_reward:.3f}, '
        template += 'mean reward: {reward_mean:.3f} [{reward_min:.3f}, {reward_max:.3f}], '
        template += 'mean action: {action_mean:.3f} [{action_min:.3f}, {action_max:.3f}], '
        template += 'mean observation: {obs_mean:.3f} [{obs_min:.3f}, {obs_max:.3f}],'
        template += '{metrics}, '
        template += 'mean epsilon: {eps_mean:.3f} [{eps_min:.3f}, {eps_max:.3f}], '
        template += 'lr: {lr:.6f}'
        # template += 'mean lr: {lr_mean:.6f} [{lr_min:.6f}, {lr_max:.6f}]'
        ...
        variables = {
            ...
            'eps_mean': np.mean(self.eps[episode]),
            'eps_min': np.min(self.eps[episode]),
            'eps_max': np.max(self.eps[episode]),
            'lr': self.lr_getter() # get just lr at the end of ep
            # 'lr_mean': np.mean(self.learn_rate[episode]),
            # 'lr_min': np.min(self.learn_rate[episode]),
            # 'lr_max': np.max(self.learn_rate[episode])
        }
        print(template.format(**variables))
    ...
    def on_step_end(self, step, logs):
        ...
        self.step += 1
        # custom
        self.eps[episode].append(logs['epsilon'])
        # self.learn_rate[episode].append(self.lr_getter())
...
class FileLogger(Callback):
    ...
    def on_train_end(self, logs):
        ...
        print("Log saved to {}.".format(self.filepath))
        self.save_data()
...
#     def on_episode_end(self, episode, logs):
#         # Get vals
#         lr = self.lr_getter(self.model.optimizer)()
#         self.learn_rate[episode] = lr
#         # decay = self.model.optimizer.decay
#         # lr = self.model.optimizer.lr
#         # iters = self.model.optimizer.iterations # only this should not be const
#         # beta_1 = self.model.optimizer.beta_1
